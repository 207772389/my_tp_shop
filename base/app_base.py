...
class AppBase(Base):
    ...
    def app_base_right_wipe_left(self,loc_area,click_text):
        ...
        while True:
            ...
            try:
                log.info("开始查找要点击的元素：{}".format(el))
                #查找元素时 记得修改等待时间
                el = self.base_find_element(loc,timeout=3)
                #如果没有报错 就说明找到了
                #输出提示信息
                log.info("找到{}元素啦！".format(el))
                ...
                #没有找到元素
                print("未找到{}元素".format(el))
                log.info("未找到{}元素，需要执行滑动操作了。。。".format(el))
                #滑动屏幕 继续寻找
            ...
            if page_source == self.driver.page_source:
                ...
                print("滑动找了一遍，还是没有找到{}元素".format(el))
                log.info("滑动找了一遍，还是没有找到{}元素".format(el))
                #抛异常
    ...
    def app_base_down_wipe_up(self,loc_area,click_text):
        ...
        while True:
            ...
            try:
                ...
                #没有找到元素
                print("未找到{}元素".format(el))
                log.info("未找到{}元素，需要执行上下滑动操作了。。。".format(el))
                #滑动屏幕 继续寻找
                self.driver.swipe(start_x,start_y,end_x,end_y,duration=2000)
                log.info("向上滑动了一次哦！！！")
            #判断是否为最后一页 重新获取一次页面结果 和上次的比对来判断是否为最后一页
